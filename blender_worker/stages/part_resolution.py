...
from vcf_core.rigging import PartCandidate, get_rig_template, resolve_parts
...
def resolve_scene_parts(objects: list[bpy.types.Object], job: dict) -> tuple[dict, list[dict]]:
    ...
    resolutions, traces = resolve_parts(
        [_candidate(obj) for obj in objects], part_overrides=job.get("part_overrides", {}), template=template,
    )
...
def render_part_map_preview(objects: list[bpy.types.Object], path: Path) -> None:
    ...
    try:
        for obj, _original in saved:
            ...
            color = PART_COLORS.get(role, "#FF00FF")
